...
export const CriarQuadrados = (VezJogador:string, Button:boolean, Quadrado: (VezJogador:string, Button:boolean) => JSX.Element) => {
  const Quadrados = [];
  for(let i=0; i<9; i++){
    Quadrados.push(Quadrado(VezJogador, Button));
  }
  return Quadrados;
}
...
export const ReiniciarJogo = (Reiniciar:number, setReiniciar:React.Dispatch<React.SetStateAction<number>>) => {
  for(let i=0; i<9; i++){
